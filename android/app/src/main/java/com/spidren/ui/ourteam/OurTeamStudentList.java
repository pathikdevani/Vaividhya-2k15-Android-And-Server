...
public class OurTeamStudentList extends GridView{
	...
	public OurTeamStudentList(Context context, int type, int width, int height, int x, int y) {
		super(context);
	...
	private class Adapter extends BaseAdapter{
		...
		private ArrayList<DBStudent> data;
		...
		public Adapter(Context context,int width) {
			...
			data = MainActivity.db.getStudentList();
			
			MainActivity.macros.addMacro(new Macro(MacroTag.NOTIFY_STUDENT_LIST) {
		...
		private class Iteam extends MyRelativeLayout{
			...
			public void setData(DBStudent student){
				ImageLoader.getInstance().displayImage(Config.student_image_link(student.getId()), profile);
				post.setText(Html.fromHtml(student.getPost()));
